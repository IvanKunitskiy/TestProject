...
@Owner("Dmytro")
public class C26607_SwipeMerchantAuthTest extends BaseTest {
    ...
    private double holdAmount = 100.00;
    ...
    @TestRailIssue(issueID = 26607, testRunName = TEST_RUN_NAME)
    @Test(description = "C26607, Swipe Merchant Auth")
    @Severity(SeverityLevel.CRITICAL)
    public void verifySwipeMerchantAuth() {
        ...
        logInfo("Step 4: Search for CHK account that is assigned to the Debit Card from the precondition and verify its: \n" +
                "- current balance \n" +
                "- available balance \n");
        expectedBalanceData.reduceAvailableBalance(holdAmount);
        Actions.clientPageActions().searchAndOpenClientByName(chkAccountNumber);
        BalanceDataForCHKAcc actualBalanceDataForCHKAcc = AccountActions.retrievingAccountData().getBalanceDataForCHKAcc();
        Assert.assertEquals(actualBalanceDataForCHKAcc.getCurrentBalance(), expectedBalanceData.getCurrentBalance(), "CHK account current balance is not correct!");
        Assert.assertEquals(actualBalanceDataForCHKAcc.getAvailableBalance(), expectedBalanceData.getAvailableBalance(), "CHK account available balance is not correct!");
        ...
        Assert.assertEquals(holdInstructionAmount, holdAmount,
        ...
        Assert.assertEquals(Double.parseDouble(transactionAmount), holdAmount,
