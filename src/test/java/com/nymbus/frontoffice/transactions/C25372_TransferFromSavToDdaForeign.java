...
public class C25372_TransferFromSavToDdaForeign extends BaseTest {
    ...
    public void preCondition() {
        ...
        double foreignFeeValue = Actions.nonTellerTransactionActions().getForeignFeeAndLogoutFromWebAdmin(1);
    ...
    public void transferFromSavToDdaForeign() {
        ...
        Assert.assertTrue(Actions.transactionActions().isTransactionCodePresent(TransactionCode.ATM_DEBIT_MEMO_219.getTransCode().split("\\s+")[0]),
                "219 - Debit Memo is present in transaction list");
        Assert.assertTrue(Actions.transactionActions().isTransactionCodePresent(TransactionCode.ATM_USAGE_229_FEE.getTransCode().split("\\s+")[0]),
