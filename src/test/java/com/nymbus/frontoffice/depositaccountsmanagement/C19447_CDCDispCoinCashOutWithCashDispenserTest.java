...
public class C19447_CDCDispCoinCashOutWithCashDispenserTest extends BaseTest {
    ...
    public void preConditions() {
        ...
        cashOutDestination.getDenominationsHashMap().put(Denominations.HALFDOLLARS, 10d);
        ...
        // Get cash dispenser data and cash drawer data
    ...
    public void cDCDispCoinCashOutWithCashDispenserTest() {
        ...
        logInfo("Step 2: Go to Teller screen and log in to proof date with:\n" +
                ...
                "- cash dispenser from preconditions (select any side)");
        Actions.transactionActions().goToTellerPage();
        ...
        logInfo("Step 4: Specify coin denomination with amount > 0.99 and check [OK] button");
        logInfo("Step 5: Fill in Cash Denominations with amount that is > $0.00 with amount < or equal to Available Balance of Account's from Precondition#3 and click OK button");
        Actions.transactionActions().setCashOutDestination(cashOutDestination);

        logInfo("Step 6: Verify GL account populated for Cash Out item");
        ...
        logInfo("Step 7: Select the following fund type in the Source\n" +
                "- Source: Withdrawal");
        logInfo("Step 8: Select account from preconditions and specify amount = Cash Out item amount, that can be covered by money in cash drawer's denominations");
        Actions.transactionActions().setWithDrawalSource(withdrawalSource, 0);

        logInfo("Step 9: Click [Commit Transaction] button");
        Pages.tellerPage().clickCommitButton();
        Pages.verifyConductor().waitModalWindow();

        logInfo("Step 10: Specify any client in the opened verify popup (e.g. owner of the withdrawal account) and submit verify screen");
        Pages.verifyConductor().clickVerifyButton();
        Pages.tellerPage().waitForPrintReceipt();
        Pages.tellerPage().closeModal();

        logInfo("Step 11: Go to Cash Drawer screen, search for cash Dispenser used in teller session and verify its:\n" +
                "- denominations\n" +
                "- total cash out");
        Pages.aSideMenuPage().clickCashDrawerMenuItem();
        SelenideTools.sleep(Constants.MINI_TIMEOUT);
        ...
        logInfo("Step 12: Go to Cash Drawer screen, search for cash drawer used in teller session and verify its:\n" +
                "- denominations\n" +
                "- total cash out");
        Actions.cashDrawerAction().selectSpecificCashDrawer(Constants.USERNAME);
        SelenideTools.sleep(Constants.MICRO_TIMEOUT);
        ...
        logInfo("Step 13: Go to account used in withdrawal item and verify its:\n" +
                "- current balance\n" +
                "- available balance");
        Pages.aSideMenuPage().clickClientMenuItem();
        Actions.clientPageActions().searchAndOpenAccountByAccountNumber(chkAccount);
        ...
        logInfo("Step 14: Open account on the Transactions tab and verify the committed transaction");
