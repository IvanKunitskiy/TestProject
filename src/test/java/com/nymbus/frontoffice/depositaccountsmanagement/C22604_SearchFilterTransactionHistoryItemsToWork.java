...
public class C22604_SearchFilterTransactionHistoryItemsToWork extends BaseTest {
    ...
    public void preCondition() {
        ...
        int index = 1;
        number = WebAdminPages.rulesUIQueryAnalyzerPage().getAccountNumberSecByIndex(index);
        amount = WebAdminPages.rulesUIQueryAnalyzerPage().getAmount(index);
        String balance = WebAdminPages.rulesUIQueryAnalyzerPage().getBalanceByIndex(index);
        effectiveDate = WebAdminPages.rulesUIQueryAnalyzerPage().getEffectiveDate(index);
        while (Double.parseDouble(amount) > Double.parseDouble(balance)) {
            index++;
            number = WebAdminPages.rulesUIQueryAnalyzerPage().getAccountNumberSecByIndex(index);
            amount = WebAdminPages.rulesUIQueryAnalyzerPage().getAmount(index);
