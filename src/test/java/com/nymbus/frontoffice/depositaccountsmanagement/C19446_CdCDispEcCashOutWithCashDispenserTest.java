...
public class C19446_CdCDispEcCashOutWithCashDispenserTest extends BaseTest {
    ...
    @TestRailIssue(issueID = 19446, testRunName = TEST_RUN_NAME)
    @Test(description = "C19446, [CD+CDisp] EC Cash Out with Cash Dispenser")
    @Severity(SeverityLevel.CRITICAL)
    public void cdCDispEcCashOutWithCashDispenserTest() {
        logInfo("Step 1: Log in to the system as the user from preconditions");
        Actions.loginActions().doLogin(userCredentials.getUserName(), userCredentials.getPassword());

        logInfo("Step 2: Go to Journal screen and log in to proof date with teller + cash drawer + cash dispenser mentioned above");
