...
public class WebAdminUsersActions {
    ...
    private String getCashDispenserNameUrl() {
        return Constants.WEB_ADMIN_URL
                + "RulesUIQuery.ct?waDbName=fnbuatcoreDS&dqlQuery=count%3A+10%0D%0Afrom%3A+bank.data.cashdrawer"
                + "%0D%0Awhere%3A+%0D%0A-+.cashdrawertype->name%3A+Cash+Dispenser%0D%0A-" +
                "+.locationid->name%3A+Clarence+Office++%23Current+User%27s+location&source=";
    }
    ...
    private String getLoanAccountDataUrl(String accountNumber) {
        return Constants.WEB_ADMIN_URL +
                "RulesUIQuery.ct?" +
                "waDbName=nymbusdev12DS&" +
                "dqlQuery=count%3A+10%0D%0A" +
    ...
    private String getInterestNextPaymentUrl(String accountNumber) {
        return Constants.WEB_ADMIN_URL +
                "RulesUIQuery.ct?" +
                "waDbName=coreDS&" +
                "dqlQuery=count%3A+10%0D%0A" +
                "from%3A+bank.data.actloan%0D%0A" +
                "select%3A+rootid%2C+createdwhen%2C+createdby%2C+accountid%2C+nextduedate%2C+interestnextpaymentdate%0D%0A" +
                "where%3A%0D%0A" +
                "-+.accountid" +
                "-%3Eaccountnumber%3A+" +
                accountNumber +
    ...
    private AccountData getLoanAccountDataFromQueryByUrl(String url) {
        ...
        accountData.setPayoffAmount(WebAdminPages.rulesUIQueryAnalyzerPage().getPayoff());
        accountData.setTotalPastDue(WebAdminPages.rulesUIQueryAnalyzerPage().getTotalPast());
        accountData.setPrincipalNextDue(WebAdminPages.rulesUIQueryAnalyzerPage().getPrincipalNextDue());
        accountData.setInterestNextDue(WebAdminPages.rulesUIQueryAnalyzerPage().getInterestNextDue());
        accountData.setTotalNextDue(WebAdminPages.rulesUIQueryAnalyzerPage().getTotalNextDue());
    ...
    public String getInterestNextPaymentDate(String accountNumber) {
        SelenideTools.openUrl(getInterestNextPaymentUrl(accountNumber));
        WebAdminPages.rulesUIQueryAnalyzerPage().waitForPageLoad();
        WebAdminPages.rulesUIQueryAnalyzerPage().waitForSearchResultTable();

        return WebAdminPages.rulesUIQueryAnalyzerPage().getInterestNextPaymentDateByIndex(1);
    }
    ...
    public String getLoanAccountWithDaysLateAboveOrEqualTwo() {
        SelenideTools.openUrl(getLoanAccountWithDaysLateAboveOrEqualTwoUrl());
        return WebAdminPages.rulesUIQueryAnalyzerPage().getLoanAccountNumberValueByIndex(3);
    }

    public String getCashDispenserNameByIndex(int index){
        SelenideTools.openUrl(getCashDispenserNameUrl());
